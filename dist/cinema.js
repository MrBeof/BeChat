(() => {
  ...
  function render() {
    ...
    $('#cinema').innerHTML=`<div class="t-page">${view.error?`<div class="t-status" role="alert">${escape(view.error)} <button class="t-link" data-refresh>Tekrar dene</button></div>`:''}${view.loading?'<p class="t-note" role="status">Listelerin eşitleniyor…</p>':''}${content}<footer class="t-footer"><span>🦊 Tilki · Birlikte izlenecek çok hikâye var.</span><span>${demo()?'Demo hesabı · Veriler bu oturumda tutulur':'Alfa 0.3 · Özenle seçilmiş 12 yapım'}</span></footer></div>`;
  ...
  async function mutateLibrary(id,changes,remove=false) {
    ...
    if(!demo()) {const {error}=await(remove?client().from('cinema_library').delete().eq('owner_id',user.id).eq('movie_id',id):client().from('cinema_library').upsert(row));if(error)throw error;}
  ...
  async function searchPeople(event) {
    ...
    try {
      if(demo())view.people=bridge().contacts.filter(p=>p.display_name.toLocaleLowerCase('tr-TR').includes(query.toLocaleLowerCase('tr-TR')));
      else {const safe=query.replace(/[\\%_]/g,'');const {data,error}=await client().from('profiles').select('id,display_name,status').neq('id',account().id).ilike('display_name',`%${safe}%`).limit(20);if(error)throw error;if(version!==view.version||lookup!==view.lookup)return;view.people=(data||[]).filter(x=>!bridge().blocked.includes(x.id));}
  ...
  async function friendAction(action,id) {
    ...
    if(demo()) {
      if(action==='add'){const p=view.people.find(x=>x.id===id);if(!p)return;view.friends.push({id:crypto.randomUUID(),requester_id:user.id,recipient_id:id,status:'accepted',requester:user,recipient:p});}
      else if(action==='delete')view.friends=view.friends.filter(x=>x.id!==id);
      else view.friends.find(x=>x.id===id).status='accepted';
      render();return toast('Demo arkadaş listen güncellendi.');
    }
    let result;
  ...
  async function saveProfile(event) {
    ...
    try{if(!demo()){const {error}=await client().from('profiles').update(row).eq('id',user.id);if(error)throw error;}if(version!==view.version)return;Object.assign(user,row);nav();render();toast('Profilin güncellendi.');}catch(error){toast('Profil kaydedilemedi: '+error.message);}finally{button.disabled=false;}
  }
